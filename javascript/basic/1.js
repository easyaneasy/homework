...
// 태그 : HTML 요소의 시작과 끝을 나타내는 표식
// 속성 : 시작태그 안에 위치하며 요소에 대한 추가 정보나 요소의 동작이나 스타일을 적용
// 컨텐츠 : 시작태그와 종료태그 사이에 포함되는 실제 내용

// 문제 [개념]
// HTML 속성 중 class 속성의 역할에 대해 작성한다

// 여러 요소에 동일한 스타일을 적용할 수 있게 한다
...
// let(변수)은 재할당이 가능하고 const(상수)는 재할당이 불가능하다
...
// a >= b : a가 b보다 크거나 같다
// a <= b : a가 b보다 작거나 같다
// a === b : a와 b가 자료형과 값 모두 같다
// a !== b : a와 b가 자료형 또는 값이 다르다
...
// 배열의 생성 방법 : 대괄호[]를 사용해 생성한다. 요소는 쉼표로 구분한다.
// 배열 요소 접근 방법 : 인덱스를 사용해 접근한다. 인덱스는 0부터 시작한다.
// 배열 요소 추가 방법 : .push()는 배열의 맨 뒤에 .unshift()는 배열의 맨 앞에 .splice()는 특정 위치에 추가한다.
// 배열 요소 삭제 방법 : .pop()은 배열의 끝 요소를 제거하고 .shift()는 배열의 첫 요소를 제거하고 반환한다. 
// .splice()는 배열의 중간 특정 인덱스부터 원하는 개수만큼 요소를 삭제한다.
...
// 객체의 생성 방법 : 중괄호{}를 사용해 생성한다. 키와 값은 콜론:으로 속성 쌍은 쉼표,로 구분한다.
// 객체 속성 접근 방법 : 마침표.또는 대괄호[]표기법을 사용해 접근한다.
// 객체 속성 추가 방법 : 할당 연산자를 사용하여 존재하지 않는 새로운 키에 값을 할당하면 속성이 추가된다.
// 객체 속성 삭제 방법 : delete 연산자를 사용한다.
...
// 문제 [실습]
// 변수 object2에 속성명(key) job, 속성값(value) "개발자" 인 속성을 추가한다
...
function processScore(student){
    // sum
    const totalSum = student.score.reduce((acc, current) => {
        return acc + current;
    },0);
    student.sum = totalSum;

    // average
    const averageScore = totalSum / student.score.length;
...
// map() : 새로운 값을 담은 배열을 반환
// filter() : 원하는 값만 모은 새로운 배열을 반환

// 문제 [개념]
// 배열의 고차 메서드 map()과 filter()의 차이점을 작성한다

// map()은 새로운 값을 추가한 새로운 배열을 반환하고 filter()는 원하는 값만 모은 새로운 배열을 반환한다.
...
let newNumbers = numbers3.map((num) => {
    return num ** 2;
})
...
let newNumbers2 = numbers4.filter((num) => {
    return num % 2 === 0;
});
...
// 펼침 연산자고 배열이나 객체의 요소를 개별적인 요소로 펼쳐주는 역할을 한다. 원본 데이터 변경 X
...
console.log(array1 === array22); // false
...
// 구조 분해 할당이란 배열이나 객체의 속성들을 해체하여 그 값을 개별적인 변수에 쉽고 간결하게 할당할 수 있게 해주는 문법이다.
