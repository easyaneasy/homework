...
function getIntersection(arr1, arr2){
    ...
    return arr1.filter((e) => {return set2.has(e)});
}
...
function formatToKRW(num){
    const formatter = new Intl.NumberFormat('ko-KR', {
        ...
        currency: 'KRW',    // 한국 원화 지정
        minimumFractionDigits: 0, // 소수점 최소 자릿수 (0으로 설정하여 정수만 표시)
    });
...
function invertObject(obj){
    return Object.entries(obj).reduce((invertedObj, entry) => {
...
function findTopStudent(students, subject){
    const topStudent = students.reduce((maxStudent, currentStudent) => {
        const currentScore = currentStudent.scores[subject] || 0;
        const maxScore = maxStudent.scores[subject] || 0;
...
function groupByCategory(arr, key = 'category'){
