...
function areAllNumbersGreaterThan(arr, target){
    return arr.every((num) => {return num > target});
}
...
function countStringOccurrences(arr){
    return arr.reduce((acc, currentString) => {
        const currentCount = acc[currentString] || 0;
        acc[currentString] = currentCount + 1;
        return acc;
    }, {});
}
...
function getProductsByCategory(products, str){
    return products.filter((product) => {return product.category === str});
}
...
function getValuesByKey(arr, key){
    return arr.map((user) => {return user[key]});
}
...
function groupByKey(arr, key){
    return arr.reduce((groups, currentObj) => {
        const groupName = currentObj[key];
        if (!groups[groupName]) {
            groups[groupName] = [];
        }
        groups[groupName].push(currentObj);
        return groups;
    }, {});
}
