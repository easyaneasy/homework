...
function getHighScoreStudents(students1){
    ...
    for(let i = 0; i < students1.length; i++){
        if(students1[i]["score"] >= 80){
            students1_1.push(students1[i]["name"]);
        }
    }
    return students1_1;
}
...
const getHighScoreStudentsArrow = (students2) => {
    ...
    for(let i = 0; i < students2.length; i++){
        if(students2[i]["score"] >= 80){
            students2_1.push(students2[i]["name"]);
        }
    }
    return students2_1;
}
...
function getLowStockProducts(products1){
    ...
    for(let i = 0; i < products1.length; i++){
        if(products1[i]["stock"] < 10){
            products1_1.push(products1[i]["name"]);
        }
    }
    return products1_1;
}
...
const getLowStockProductsArrow = (products2) => {
    ...
    for(let i = 0; i < products2.length; i++){
        if(products2[i]["stock"] < 10){
            products2_1.push(products2[i]["name"]);
        }
    }
    return products2_1;
}
...
function transformNumbers(numbers6){
    ...
    for(let i = 0; i < numbers6.length; i++){
        if(numbers6[i] <= 10){
            if(numbers6[i]%2 === 0){
                numbers6_1.push(numbers6[i]*2)
            }else{
                numbers6_1.push(numbers6[i]+3)
            }
        }else{
            numbers6_1.push(numbers6[i]);
        }
    }
    return numbers6_1;
}
...
const transformNumbersArrow = (numbers7) => {
    ...
    for(let i = 0; i < numbers7.length; i++){
        if(numbers7[i] <= 10){
            if(numbers7[i]%2 === 0){
                numbers7_1.push(numbers7[i]*2)
            }else{
                numbers7_1.push(numbers7[i]+3)
            }
        }else{
            numbers7_1.push(numbers7[i]);
        }
    }
    return numbers7_1;
}
...
function classifyStudentsByGrade(students3){
    let students3_1 = {A: [], B: [], C: [], D: []};
    // 객체의 속성을 배열로 생성

    for(let i = 0; i < students3.length; i++){
        if(students3[i]["score"] >= 90){
            students3_1["A"].push(students3[i]["name"]);
        }else if(students3[i]["score"] >= 80){
            students3_1["B"].push(students3[i]["name"]);
        }else if(students3[i]["score"] >= 70){
            students3_1["C"].push(students3[i]["name"]);
        }else {
            students3_1["D"].push(students3[i]["name"]);
        }
    }
    return students3_1;
}
...
const classifyStudentsByGradeArrow = (students4) => {
    let students4_1 = {A: [], B: [], C: [], D: []};
    // 객체의 속성을 배열로 생성

    for(let i = 0; i < students4.length; i++){
        if(students4[i]["score"] >= 90){
            students4_1["A"].push(students4[i]["name"]);
        }else if(students4[i]["score"] >= 80){
            students4_1["B"].push(students4[i]["name"]);
        }else if(students4[i]["score"] >= 70){
            students4_1["C"].push(students4[i]["name"]);
        }else {
            students4_1["D"].push(students4[i]["name"]);
        }
    }
    return students4_1;
}
...
function countElements(fruits1) {
    ...
    for (let i = 0; i < fruits1.length; i++) {
      let item = fruits1[i];
      if (counts[item]) {
        counts[item]++; // 이미 있으면 카운트 증가
      } else {
        counts[item] = 1; // 처음 등장하면 1로 초기화
      }
    }
    return counts;
}
...
// 화살표 함수 정의 코드
const countElementsArrow = (numbers5) => {
    let countNum = {};
